...
#             os.mkdir(f"{PACKAGE_DIR/SessionSummaries}")
        ...
        # self.flare_trigger_lines = []
        # self.flare_realtrigger_lines = []
    ...
    def display_em(self):
        ...
        '''
        #emission measure:
        self.emgraph.setLogMode(False, True)
        ymin = np.nanmin(self.goes['emission measure'].iloc[-30:])
        ymax = np.nanmax(self.goes['emission measure'].iloc[-30:])

        self.emgraph.plotItem.vb.setLimits(
            yMin=np.log10(ymin * 0.8),
            yMax=np.log10(ymax * 1.2)
        )
        
        self.line_min_em, self.line_max_em = ymin*.7, ymax*1.3
...
#         self.flare_prediction_state("pre-launch")
   ...
   ####################################################################################
    ...
    #### adding in updates to trigger lines here!!####
    def trigger_vline(self, plotwidget, y, eve=False):
        ''' Makes a vertical line for the trigger and realtime trigger, to be used for all the different plots.
        '''
        t = pd.Timestamp(self.current_time).timestamp()
        realt = pd.Timestamp(self.current_realtime).timestamp()
        pen = pg.mkPen(color='gray', width=5)
        realpen = pg.mkPen(color='k', width=5)
        if eve:
            return plotwidget.plot([pd.Timestamp(self.flare_summary['Trigger'].iloc[-1]).timestamp()]*2, y, pen=pen), plotwidget.plot([realt]*2, y, pen=realpen)
        else:
            return plotwidget.plot([t]*2, y, pen=pen), plotwidget.plot([realt]*2, y, pen=realpen)
        
    def add_triggerlines(self):
        if self._logy:
    ...
    def FAI_vline(self, plotwidget, y):
        ''' Plots the FAI line if there is one!
        '''
        t = pd.Timestamp(self.current_time).timestamp()
        pen = pg.mkPen(color='pink', width=5)
        return plotwidget.plot([pd.Timestamp(self.goes['time_tag'].iloc[self.FAI_loc]).timestamp()]*2, y, pen=pen)
        
    def add_FAIlines(self):
        if self._logy:
            lower = self._lowest_yrange
            higher = self._highest_yrange
        else:
